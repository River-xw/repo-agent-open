...
class RepoInfoAgent(BaseAgent):
    """ Agent for collecting repository information.
    ...
    def run(self, owner: str = None, repo_name: str = None) -> dict:
        ...
        if owner and repo_name:
            ...
            prompt_parts.append("4. Set commits to empty array since remote info is not available")
    ...
    def _extract_result(self, final_state: AgentState) -> dict:
        ...
        try:
            ...
            return self._get_fallback_result(
                ...
                raw_output=str(last_message.content) if hasattr(last_message, "content") else ""
            )
...
def RepoInfoAgentTest():
    # Local repository only (no remote info)
    # agent = RepoInfoAgent(
    ...
    # With remote repository info
    agent2 = RepoInfoAgent(
        repo_path="./.repos/facebook_zstd",
        wiki_path=""
    )
    
    repo_info2 = agent2.run(owner="facebook", repo_name="zstd")
    print("\nResult:")
    print(json.dumps(repo_info2, indent=2))
